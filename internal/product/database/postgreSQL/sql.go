...
func (s *InventoryService) DisplayProducts(ctx context.Context) ([]model.Products, error) {
	productsRows, err := s.Db.Query(ctx, displayProducts)
	if err != nil {
	...
	attrs, err := s.GetAllAttributes(ctx, productIDs)
	if err != nil {
...
func (s *InventoryService) GetAllAttributes(ctx context.Context, productIDs []string) (map[string]map[string]string, error) {
	if len(productIDs) == 0 {
	...
	rows, err := s.Db.Query(ctx, query, productIDs)
	if err != nil {
...
func (s *InventoryService) AdditionProducts(ctx context.Context, products model.Products) error {
	...
	var exists bool
	err := s.Db.QueryRow(ctx, productCheckIns, products.Name).Scan(&exists)
	...
	if _, err := s.Db.Exec(ctx, addingAProducts, products.Product_id, products.Name, products.Description, products.Weight, products.Barcode); err != nil {
		return fmt.Errorf("Ошибка с добавлением товара: %w", err)
	}

	if _, err := s.Db.Exec(ctx, deleteKeyValue, products.Product_id); err != nil {
		return fmt.Errorf("Ошибка с удалением ключ значения: %w", err)
	}

	for _, m := range products.KeyValue {
		for key, value := range m {
			if _, err := s.Db.Exec(ctx, addingKeyValue, products.Product_id, key, value); err != nil {
...
func (s *InventoryService) UpdateProd(ctx context.Context, products model.Products) error {

	var exists bool
	err := s.Db.QueryRow(ctx, productCheckUpd, products.Product_id).Scan(&exists)
	...
	result, err := s.Db.Exec(ctx, updateAProducts, products.Description, products.Product_id)
	...
	for key, value := range products.KeyValue {
		if _, err := s.Db.Exec(ctx, updateValue, value, products.Product_id, key); err != nil {
			return err
		}
	}
