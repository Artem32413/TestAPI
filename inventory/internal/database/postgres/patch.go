...
func (d *DBService) UpdateInventorySQL(log *zap.Logger, ctx context.Context, inventory structs.Inventory) error {
	...
	if exist := attributes.Exists(log, d.db, ctx, 1, inventory, structs.NewInventoryDiscount{}, structs.NewInventory{}, structs.WarehousePagination{}); !exist {
...
func (d *DBService) DiscountInventorySQL(log *zap.Logger, ctx context.Context, discount structs.NewInventoryDiscount) error {
	...
	if exist := attributes.Exists(log, d.db, ctx, 3, structs.Inventory{}, discount, structs.NewInventory{}, structs.WarehousePagination{}); !exist {
...
func (d *DBService) CountPriceSQL(log *zap.Logger, ctx context.Context, count structs.NewInventory) (structs.SummingUp, error) {
	...
	if exist := attributes.Exists(log, d.db, ctx, 4, structs.Inventory{}, structs.NewInventoryDiscount{}, count, structs.WarehousePagination{}); !exist {
	...
	productIDs, quantities := attributes.Slices(count)
